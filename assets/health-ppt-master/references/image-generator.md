...
- This DSH pilot does not package or inject provider credentials. Image generation remains unavailable until a trusted deployment plugin explicitly provides the selected backend and key; do not rely on inherited parent-process variables or create an `.env` inside the package.

Precedence:
- Deployment-injected values own backend selection and credentials.
...
> Use provider-specific names only (e.g. `GEMINI_API_KEY`, `OPENAI_API_KEY`). See `.env.example` in clone mode or `<skill-dir>/.env.example` in skill-install mode for the full set per backend.

...
python3 <skill-dir>/scripts/animation_config.py scaffold <project>

# Validate references before export
python3 <skill-dir>/scripts/animation_config.py validate <project>

# Export reads <project>/animations.json automatically when present
python3 <skill-dir>/scripts/svg_to_pptx.py <project>
```
...
python3 <skill-dir>/scripts/svg_to_pptx.py <project> -t push --transition-duration 0.6

# Disable
python3 <skill-dir>/scripts/svg_to_pptx.py <project> -t none

# Auto-advance every 5 seconds (kiosk-style playback)
python3 <skill-dir>/scripts/svg_to_pptx.py <project> --auto-advance 5
```
...
python3 <skill-dir>/scripts/svg_to_pptx.py <project> -a none

# Use a single effect (still cascades via the default after-previous trigger)
python3 <skill-dir>/scripts/svg_to_pptx.py <project> --animation fade

# Switch to on-click for live presentations (presenter controls pacing)
python3 <skill-dir>/scripts/svg_to_pptx.py <project> --animation-trigger on-click

# Custom pacing
python3 <skill-dir>/scripts/svg_to_pptx.py <project> --animation mixed \
        --animation-stagger 0.7 --animation-duration 0.5

# All groups animate in unison on slide entry
python3 <skill-dir>/scripts/svg_to_pptx.py <project> --animation-trigger with-previous
```
...
Executors should wrap logical sections in `<g id>` regardless of whether you plan to animate. The Executor reference (`<skill-dir>/references/shared-standards.md`) requires it.

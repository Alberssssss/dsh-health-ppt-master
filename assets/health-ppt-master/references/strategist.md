...
|--------------|---------|-----------|
| Project creation + non-blocking source/template analysis | **Strategist**: single Final Build Confirmation + Design Spec | Image_Generator or Executor |
...
## 0. Medical Scenario Pre-scan (this fork)

🩺 **Run this before the Final Build Confirmation.** This is the medical fork's content lever.

1. `read_file references/medical-scenarios.md`.
2. Classify the source / topic into **one** scenario (clinical_case, teaching_rounds, nsfc_grant, academic_conference, journal_club, cme_course, title_promotion, hospital_accreditation, drug_device_filing) — or conclude "no medical match".
3. When a scenario matches, carry its guidance into the confirmations below:
   - **§IX Content Outline** — seed from the playbook's outline skeleton (adapt depth to the real source; never invent clinical facts — mark gaps "待补充").
   - **Confirmation c/d** — audience/occasion and structure from the playbook.
   - **Confirmation e (Color)** — the playbook's palette (medical blue+green / navy+teal / institutional blue+crimson / teaching green+amber).
   - **§VII Visualization** — pre-select the playbook's charts; for research/RCT/diagnostic content prefer `templates/charts/survival_curve_km.svg`, `forest_plot.svg`, `consort_flow.svg`, `roc_curve.svg`.
     - 🩺 **journal_club 图表保真闸 (ADR-0048)**: 文献汇报的结果/图表页 **默认嵌入原文原图**（document-parser 抽取的 `Extracted` §VIII 图像行，`Acquire Via: source`，no-crop 逐像素），**不是**用上面的 chart 模板重绘。上面的模板预选在 journal_club 下**仅作兜底**：(a) 原文没有该数据的独立图，或 (b) 汇报人自己的二次分析（PICO/CASP 核查表、本研究 vs 既往对比）。原文若本身是森林图 / KM 曲线，**直接嵌原图**，不要用 `forest_plot.svg` / `survival_curve_km.svg` 重画。重绘允许但必须用原文精确数值并标注"据原文重绘"；严禁 AI 示意图替代原文图。抽取残缺时走 [`executor-base.md` §6 的 render-crop 路径](./executor-base.md) 拿干净原图。
   - **Template — you MUST recommend the playbook's finished DECK first** (not the layout). A **deck** is ready-to-use (identity + structure + look bundled into one finished asset) and is the only "上手即用" preset; a **layout** is a bare skeleton that drops the deck's finished polish (seal ring / gold rule / metric cards). Use this scenario→DECK map (memorize it — do not reach for a layout name):

     | Scenario cluster | Finished **DECK** (recommend this path) | Layout (fallback only) |
     |---|---|---|
     | 临床病例 / 教学查房 (clinical_case, teaching_rounds) | `templates/decks/clinical_report/` | `medical_university` |
     | 科研学术 / 文献汇报 / 大会发言 (academic_conference, journal_club) | `templates/decks/research_academic/` | `medical_research` |
     | 教辅 / CME / 进修培训 (cme_course) | `templates/decks/medical_education/` | `medical_university` |
     | 院内汇报 / 质控 / 等级评审 / 标书 / 职称 / 药械申报 (hospital_accreditation, nsfc_grant, title_promotion, drug_device_filing) | `templates/decks/institutional_official/` | `medical_official` |

     ⚠️ **Naming trap**: the management/official deck is named `institutional_official` — it is **NOT** the layout `medical_official`. They serve the same scenario under different names; always name the **deck** path in the Final Build Confirmation. Copy it **only after that single package is accepted** (`cp -r templates/decks/<id>/* <project_path>/templates/`); a deck locks all segments, narrowing the execution fields to audience / page count / outline / tone. Reach for the structure-only **layout** (`templates/layouts/medical_*/`) only when the doctor explicitly wants "just the structure / I'll bring my own brand".

> **Skip-detection signal — deck-first self-audit**: if your bundled confirmations name a `medical_*` **layout** (medical_official / medical_research / medical_university) without having first offered the matching finished **deck** from the table above, you skipped deck-first — re-read the scenario→DECK map and lead with the deck. Naming a layout is correct only when the doctor explicitly asked for "just the structure" or is bringing their own brand. This respects the ⛔ BLOCKING gate and does not override SKILL.md Step 3's explicit-path discipline.
4. **Bring-your-own template.** If the doctor refers to or attaches their own hospital / department template (a branded PPT, slide screenshots, or a logo) instead of wanting a preset, do **not** force a preset deck — point them to [`../workflows/medical-byo-template.md`](../workflows/medical-byo-template.md) to reverse-engineer it into a reusable medical deck / brand, then continue with that as the locked template.
5. **No match → no force-fit.** Drop back to the generic flow and treat as free design. Routing a non-medical deck through a clinical skeleton is a soft error.

> Hard rule (medical): doses, lab values, statistics, guideline grades, and citations come only from the source — do not fabricate plausible-looking clinical data to fill a slide. **For scenarios that reproduce a source paper's own evidence (esp. `journal_club`): figures/tables MUST be the ORIGINAL extracted images (`Extracted` status), not redrawn charts or 示意图 — a redrawn chart with correct numbers is still lower fidelity than the original. Redraw only when no clean original is obtainable (see [`executor-base.md` §6](./executor-base.md) render-crop path first), use the paper's exact reported values verbatim (never estimate off a figure), and label it a reconstruction (ADR-0048).**

---

## 0.1 journal_club Single-Package Intake (this fork · ADR-0049 amended by ADR-0078)

🩺 **Run this ONLY when §0 classified the source as `journal_club`.** Do not call `clarify` and do not ask
these questions before the Final Build Confirmation. Resolve what the user already stated, infer the rest,
and place all five levers together inside that one package's **Content Contract**:

| # | Lever | Recommended default when absent | Feeds |
|---|---|---|---|
| 1 | **受众 & 场景** | 科室读书会；专业术语保留，评价直接 | Execution Spec c（受众/场合） |
| 2 | **时长 / 页数** | 深评 15–18 页（按源文长度可缩放） | Execution Spec b（页数） |
| 3 | **汇报重心** | 方法学批判 + 统计严谨 + 临床可落地；既往证据对比为次重点 | Deck Plan + §IX 各页权重 |
| 4 | **必嵌的原图 / 表清单** | `images/image_manifest.json` 中完整 Extracted 图表默认全嵌；逐项列名 | §VIII `Extracted` 行 + §6 逐图保真闸目标集 |
| 5 | **延展开关** | 加“本研究 vs 既往证据”对照页，但默认仅用源文已报告数据；不另行联网 | §IX 延展页（源文数字、零编造） |

**Wiring & discipline**:
- Mark user-specified values as “已按用户要求”; mark inferred values as “推荐默认”. The user accepts or modifies them together with the rest of the Final Build Confirmation.
- Never emit a preliminary “journal-club scope confirmation”. `APPROVAL_BUDGET=1` applies to this scenario too.
- **图表保真闸的目标集 = lever④ 的必嵌清单 ∪ `image_manifest.json` 抽取图**。执行器对这个集合里的**每一张** Extracted 图逐一走 [`executor-base.md` §6 的嵌前逐图完整性确认门](./executor-base.md)——**确定性 `render_pdf_region.py --verify-crop`（PASS 才嵌）为准，vision 仅在可用时作可选交叉核对**（本部署主模型多无会话内 vision，不得把“看图”当门，ADR-0050）。

---

## 1. Final Build Confirmation Process
...
⛔ **BLOCKING USER APPROVAL — FINAL BUILD CONFIRMATION**: After the read, present one complete package containing Content Contract + Deck Plan + all eight execution items below + Execution Note. Wait for explicit acceptance or modification. A modified package remains this same gate; there is no later approval gate.

> **Execution discipline**: This is the first and last user-approval checkpoint in the pipeline (`APPROVAL_BUDGET=1`). After acceptance, set `BUILD_APPROVED`, complete the Design Spec, and proceed to image generation / SVG / post-processing without further pauses, clarification, or “continue?” questions.

> **Skip-detection signal — package integrity**: present the Content Contract and Deck Plan plus all eight items a–h verbatim from the template. Do NOT drop, merge, or rename them, and do NOT substitute content questions for a confirmation slot. Content the cover needs — `汇报周期 / 汇报人 / 科室名 / 项目名` — belongs in Content Contract / §IX outline seeding, never *in place of* item c/d/h. If the package is missing any of {content boundary, outline, canvas, page count, key info, **style objective**, color, icon, **typography + formula policy**, **image usage**, execution mode}, restore the complete package before showing it.
...
>
> **Inside the Final Build Confirmation — decide the library only. Do NOT run `ls | grep` yet.**
>
...
>
> **After Final Build Confirmation is accepted — when writing `design_spec.md` §VI / `spec_lock.md`**, then materialize the icon inventory:
>
> 3. Enumerate the concepts the deck actually needs (home, chart, users, …) based on the confirmed outline.
> 4. Search for each concept's filename in the chosen library: `ls <skill-dir>/templates/icons/<chosen-library>/ | grep <keyword>`
...
Formula rendering is part of Typography item g inside the Final Build Confirmation. Recommend one policy there; do not ask separately.
...
**Manifest step**: After Final Build Confirmation is accepted and before writing `design_spec.md`, if policy is `mixed` or `render-all` and formulas are selected:
...
#### Three-candidate section inside Final Build Confirmation (default path)

**Hard rule**: Unless the user has already named a specific rendering or palette (chat or template), include **≥3 distinct rendering × palette combinations** inside the Final Build Confirmation, mark one evidence-backed recommendation as the default, and let the user accept or modify it as part of that same package. Never open a separate image-style approval gate.
...
After the Final Build Confirmation is accepted (including any candidate modification inside that same gate), proceed to "Recording the lock" below.

#### Prompt depth for §VIII rows

> 🩺 **journal_club precedence (ADR-0048)**: a **paper figure that exists in the source PDF is an `Extracted` row** (`Acquire Via: source`, `Status: Extracted`, `no-crop`) — embedded verbatim from document-parser's `<stem>_files/*.png` (or the render-crop path), **NOT** an `Acquire Via: ai` generation row. The AI prompt-depth rule below applies ONLY to figures that have **no** extractable original and where a labeled reconstruction is authorized. Never AI-synthesize a 示意图 to stand in for a real paper figure.
...
#### hero_page suggestion (inside the same Final Build Confirmation)

Before presenting the Final Build Confirmation, scan the proposed outline and include any pages where the image makes more sense as the page's main voice than as a local block. Mark the recommended list and alternatives inside the package; there is no later confirm/edit/skip turn. After package acceptance, record the result as `page_role: hero_page` on the matching `ai` rows. Density is judgment-based — no fixed quota.

**Per hero_page title**: lock where it lives — `embedded` (fused into the image: neon, carved, smoke, 3D-lit lettering) or `none` (editable SVG title over an atmospheric backdrop, Primitive D). Default `none`; flip to `embedded` only when the words must be *part of the visual*, not merely a display font. Per page — may bake only the keyword while subtitle / date / chrome stay SVG. Surface it with the hero_page list inside the same Final Build Confirmation.
...
> 🩺 **journal_club (ADR-0048/0049)**: `Extracted` 原图（`Acquire Via: source`）**同样必须**跑 `python3 scripts/analyze_images.py <project_path>/images`（在 `import-sources --move` 之后），即使没选 B。原因见缺陷复盘：原图 box 若沿用固定图表槽几何、叠加保真强制的 `meet`(不裁) → 信箱化（实测填充率低至 50–60%）。analyze_images 从 `images/image_manifest.json` 读到每张 Extracted 图的 `display_ratio` / `pixel_width` / `pixel_height`（`ratio_source: manifest`）；把它填进该行的 **Dimensions / Ratio**，并在 `spec_lock.md images` 给该行标 ` | no-crop`。§IX 结果页据此**预留"比例 ≈ 原图比例"的 box**，让 `meet` 的信箱边接近零。
...
> 🩺 **`Extracted` 原图恒为 no-crop**：document-parser 抽取的原文图 / 表（`Acquire Via: source`）**总是**追加 `no-crop`——原图逐像素保真、按原生比例定 box，绝不裁、绝不塞进不匹配的固定槽（同 formula 行处理，见 [`executor-base.md` §6/§326](./executor-base.md)）。
...
>
> **Workflow**:
> 0. 🩺 **journal_club precedence (ADR-0048)** — before matching a page to any chart template, ask: does this page's data correspond to a figure/table that exists in the source paper? If YES → **embed the `Extracted` original** (record it as a §VIII `Acquire Via: source` image row, mark the page `no-template-match` in §VII with reason `source-figure embed`) and **skip chart matching for it**. Only pages whose data has no extractable original figure proceed to template matching below (and any redraw is a labeled reconstruction using the paper's exact numbers).
...
- **数值保真（evidence-reproduction decks，尤其 journal_club）**：备注里每个量（计数 / % / n / HR/OR/RR / 95%CI / p / 事件数）**必须与该页正面数字一致、且可溯原文**。**备注数值 == 页面数值 == 原文**。不得引入页面上没有的量级 / 单位 / 量纲；把数字拼写成中文口播形式时**绝不改变量级或数量级**（"21 例"→"二十一例"，**不是**"二十一万"——真实缺陷）。逐字复述正面数值，别在讲稿里"重算"。

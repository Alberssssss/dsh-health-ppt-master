...
Defined in the Design Specification & Content Outline; each image carries an `Acquire Via` field plus a status annotation. This file is authoritative for status names and SVG embedding behavior. If image approach includes "B) User-provided": run `analyze_images.py` right after the Final Build Confirmation is accepted and complete the list before outputting the design spec.
...
| **Extracted** | **Original figure/table raster extracted verbatim from the source PDF by document-parser** (`Acquire Via: source`; lives in `../images/<ns>__*.png`, manifest `source: pdf-raster\|pdf-vector`) | Reference from `../images/`; **embed verbatim** with `preserveAspectRatio="xMidYMid meet"` (no-crop) + `data-fidelity="source"` on the `<image>` + an inline `<text>` source caption (Fig/Table № + citation). **Size the box to the figure's NATIVE ratio** (manifest `pixel_width`:`pixel_height` / `display_ratio`), **not a fixed chart slot** — see [executor-base.md §6 / no-crop rule](./executor-base.md). **MUST NOT** regenerate via `image_gen` or redraw from `templates/charts/*.svg`. For journal_club, each Extracted figure is completeness-verified before embed (per-figure gate: `render_pdf_region.py --verify-crop … --record images/figure_fidelity.json --asset <embedded filename>` must record **PASS** — deterministic, vision optional). **This is mechanically enforced (ADR-0051)**: `svg_quality_checker.py` ERRORs for any no-crop Extracted original with no recorded PASS (or a FRAGMENT) or a letterboxed box (<85% fill). This is the journal_club fidelity default — ADR-0048/0049/0050/0051. |
...
3. Executor generates SVGs (svg_output/)
   ...
   ├── Extracted (source figure) → <image href="../images/<ns>__xxx.png" data-fidelity="source" preserveAspectRatio="xMidYMid meet" .../> at NATIVE ratio (manifest pixel_width:pixel_height, not a fixed slot) + inline <text> source caption; embed verbatim, NEVER redraw; must have a --verify-crop PASS in images/figure_fidelity.json (QC-enforced)

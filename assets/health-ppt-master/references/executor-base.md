...
## 2. Execution Parameter Receipt (Mandatory, NON-BLOCKING)

Before the first SVG page, output a receipt listing the already-approved canvas dimensions, body font size,
color scheme (primary/secondary/accent HEX), and font plan. This is an execution log that prevents
spec/execution drift. **Do not phrase it as a question, ask the user to confirm it, call `clarify`, or wait
for a response.** Emit the receipt and continue immediately to the pre-generation batch read.
...
```bash
ls <skill-dir>/templates/icons/chunk-filled/ | grep home
ls <skill-dir>/templates/icons/tabler-filled/ | grep home
ls <skill-dir>/templates/icons/tabler-outline/ | grep chart
ls <skill-dir>/templates/icons/phosphor-duotone/ | grep house
ls <skill-dir>/templates/icons/simple-icons/ | grep github
```
...
| **Extracted** | Original figure from the source PDF (document-parser) | Reference from `../images/` (or `../sources/<stem>_files/`); embed **verbatim** with `preserveAspectRatio="xMidYMid meet"` (no-crop) + an inline `<text>` source caption. **Size the `<image>` box to the figure's NATIVE ratio** (from `images/image_manifest.json` `pixel_width`:`pixel_height` / `display_ratio`, or `image_analysis.csv`) — see the no-crop rule below. **Never reuse a fixed chart-slot box**: letterboxing an original figure into a mismatched wide/tall slot (empty side bands, ~50–60% fill) is a fidelity/layout defect. **An Extracted original overrides the chart-template path — never redraw it from `templates/charts/*.svg` or regenerate it via `image_gen`.** See the journal_club fidelity note below. |
...
**`no-crop` images**: when a `spec_lock.md images` entry ends with ` | no-crop`, size the container to the image's native ratio (from `analyze_images.py` / `image_analysis.csv`, `images/image_manifest.json` `display_ratio` or `pixel_width`:`pixel_height`, or file dims) and use `preserveAspectRatio="xMidYMid meet"`. Untagged entries are croppable — default to `slice`. **`Extracted` rows (source figures) are ALWAYS no-crop even if `spec_lock.md` forgot the ` | no-crop` flag** (same treatment as formula rows below) — compute the box from the manifest native dims, never from the surrounding slot.

> ### 🩺 journal_club figure fidelity (ADR-0048 / 0049 / 0050)
> For 文献汇报 (journal_club), the paper's OWN figures/tables are the evidence — embed them as **`Extracted`** originals; **never** substitute a redrawn `templates/charts/*.svg` chart or an AI 示意图. Size each to its native ratio (no-crop rule above).
>
> **⛔ Per-figure completeness gate (MANDATORY) — deterministic first, vision optional (ADR-0050).** BEFORE embedding EACH `Extracted` figure you MUST confirm it is the **COMPLETE figure** — the whole plot / all panels / its at-risk table — **not** a legend fragment, an axis sliver, a caption block, or a stray vector sub-cluster. Auto-extraction genuinely produces these (real defect: a Figure-3 swimlane came out as just its legend box). **This deployment has no reliable in-session vision** (self-hosted GLM / headless: `vision_analyze` returns a text-only fallback, no X display) — so the gate is a **deterministic script check, NOT "look at the PNG"**:
>
> ```
> python3 <document-parser-dir>/scripts/render_pdf_region.py \
>     -i sources/<paper>.pdf -p <page> --verify-crop \
>     --bbox <x0,y0,x1,y1> --frac --caption "Figure N" \
>     --record images/figure_fidelity.json --asset <the exact filename you embed>
> ```
> → prints `VERDICT: PASS` (coverage ≥ 92 % of the detected figure + caption in crop) or `VERDICT: FRAGMENT coverage=..` (exit 3), and **records the verdict** into `images/figure_fidelity.json` keyed by `--asset`. `--asset` MUST equal the `<image href>` basename you will embed (e.g. `jama__figure_2.png`), so the deck-level gate can match it. **Only embed a crop that verified PASS.** The gate's target set = the Final Build Confirmation Content Contract's must-embed list ∪ every figure in `images/image_manifest.json`.
>
> **This is mechanically enforced, not honor-system (ADR-0051).** `svg_quality_checker.py` ERRORs — and the export gate then blocks — for any `Extracted` original embedded no-crop whose `--asset` has no recorded PASS in `images/figure_fidelity.json`. So you cannot skip this step or ship a FRAGMENT: a missing/FRAGMENT verdict fails QC. Mark each such `<image>` with `data-fidelity="source"` so the checker recognizes manual render-crops that are not in `image_manifest.json`.
>
> **Verdict → action**:
> - **PASS** → embed (native-ratio box), with `data-fidelity="source"` on the `<image>`.
> - **FRAGMENT / UNKNOWN** → re-acquire via the render-crop path below (use the `--detect-figure` coords it prints), then **re-verify** (same `--asset`, so the ledger updates to PASS); repeat until PASS. Do NOT hand-roll `fitz`.
> - **genuinely unobtainable clean original** → rigorous redraw (rules below).
>
> **Optional vision cross-check**: if (and only if) a vision-capable viewer is actually available this session, you MAY additionally eyeball the PNG — but the `--verify-crop` PASS is the gate; a vision glance never *substitutes* for it and never *overrides* a FRAGMENT.
>
> **render-crop path** (also used when the `Extracted` PNG is absent or FRAGMENT):
> 1. `python3 <document-parser-dir>/scripts/render_pdf_region.py -i sources/<paper>.pdf -p <page> --detect-figure --caption "Figure N"` → prints the figure's true `--frac` bbox (deterministic, unions the page's drawings + associated text; no vision). Optionally also render the full page for reference.
> 2. Crop with those coords: `... -p <page> --bbox x0,y0,x1,y1 --frac -o images/<fig>.png --dpi 220`.
> 3. `--verify-crop` the result with `--record images/figure_fidelity.json --asset <fig>.png` (step above). On PASS, add/replace it in `design_spec.md §VIII` as an `Extracted` (`Acquire Via: source`, `no-crop`) row and embed it at native ratio with `data-fidelity="source"` on the `<image>`.
>
> **Redraw is allowed but must be rigorous** (用户纪律): only when no clean original is obtainable. Then use the paper's **exact reported values** (effect sizes / CIs / p-values / event counts / risk tables — never eyeballed "≈" readings off a curve), calibrate against the rendered original, label the slide **"据原文数据重绘 / reconstructed from source data"**, and cite the figure. A redraw that invents or estimates numbers is a fidelity violation.
...
> **⚠️ Number fidelity (spelling must not change the quantity)**: when you spell a number out for TTS, the spelled form must denote the **EXACT same quantity** shown on the slide face and reported in the source. **Never change magnitude / order of magnitude, unit, or scale** while spelling — "21 例" → "二十一例", **NOT** "二十一万" (a real defect: 21 became 210,000); "1.4%" → "百分之一点四", not "百分之十四". Every count / % / n / HR/OR/RR / 95%CI / p in a note must match a number on that slide and trace to the source. Restate slide-face numbers verbatim; do not re-aggregate or re-compute in the notes.
...
- **Magnitude drift when spelling a count/quantity** (e.g. "21 例" → "二十一万", "0.1%" → "百分之十") — the spoken number must equal the slide-face number and the source; see Number fidelity above.

...
- High-quality cloud mode: provider API key is set before use:
  ...
  - This DSH pilot does not package or inject cloud keys. Cloud mode remains unavailable until a trusted deployment plugin explicitly provides the selected credential; do not rely on inherited parent-process variables or create an `.env` inside the package.
...
```bash
python3 <skill-dir>/scripts/notes_to_audio.py --list-voices --locale <locale>
```

**ElevenLabs backend**:

```bash
python3 <skill-dir>/scripts/notes_to_audio.py --provider elevenlabs --list-voices
```

**Cloud providers using explicit voice IDs/names**:

```bash
python3 <skill-dir>/scripts/notes_to_audio.py --provider minimax --list-voices
python3 <skill-dir>/scripts/notes_to_audio.py --provider qwen --list-voices
python3 <skill-dir>/scripts/notes_to_audio.py --provider cosyvoice --list-voices
```
...
python3 <skill-dir>/scripts/notes_to_audio.py <project_path> \
...
- (If skipped embedding) one-line hint on how to embed later: `python3 <skill-dir>/scripts/svg_to_pptx.py <project_path> --recorded-narration audio`.

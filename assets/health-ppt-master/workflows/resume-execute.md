...
`SKILL.md` is normally already loaded in this session (you reached this workflow by viewing the health-ppt-master skill). If it is not, read it:

```
Read skills/research/health-ppt-master/SKILL.md
```
...
- Execution Parameter Receipt (non-blocking: echo approved values, do not ask or wait)
- Pre-generation Batch Read (every layout / chart SVG referenced in `spec_lock`)
- **Resume awareness (ADR-0053)**: run `python3 <skill-dir>/scripts/resume_status.py <project_path>` — if a **previous** Phase-B attempt already generated some pages (common: the prior session dropped mid-deck), it lists DONE / INVALID / MISSING. Generate **only** the `RESUME` set (INVALID + MISSING) in roster order; do **not** re-draw DONE pages. This is the whole point of a resumable Phase B on an endpoint whose long sessions drop — a partial `svg_output/` is progress to build on, not to discard.
- Per-page `spec_lock` re-read + sequential page generation (of the resume set)
- Quality Check Gate

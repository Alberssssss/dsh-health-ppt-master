...
>
> This private bundle proves skill discovery, narrow routing, and joint plugin lifecycle only. It does not install `document-parser`, Python packages, system binaries, image or search credentials, browser/editor access, or Hermes frontend delivery behavior. Confirm each required DSH capability and external dependency before entering its step; if one is unavailable, stop and report that dependency instead of inventing a tool or path. Store project state under the current DSH session workspace, not a Hermes home directory. Tool names such as `read_file` and `run_in_background` describe the source workflow and must be mapped to capabilities actually exposed by the active DSH composition.

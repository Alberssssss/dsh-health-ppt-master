...
def finalize_project(
    ...
    # Accumulates genuine image-embed failures (unresolved href, unreadable /
    # undecodable bitmap, zero-sized box, encode failure). Benign skips (already
    # inline, EMF/WMF native passthrough, no href) are NOT counted here. Any
    # non-zero total fails the run so a broken image ref cannot silently ship.
    total_img_errors = 0
    ...
    # Done
    if total_img_errors > 0:
        safe_print(
            f"[ERROR] {total_img_errors} image(s) failed to embed (unresolved / "
            "unreadable / zero-box / encode failure). The deck would ship with "
            "broken image references — fix the source images and re-run before export."
        )
        return False

...
def _analyze_pictures(zf: zipfile.ZipFile, slide_root: ET.Element, slide_ref: SlideRef) -> list[dict[str, Any]]:
    """Embedded ``<p:pic>`` images on the slide, for the image-edit plan.

    template-fill clones a whole source slide, so any picture here is baked into
    the output verbatim unless an ``image_edit`` clears or replaces it. Recording
    each picture's id / geometry / current media target lets the plan target it
    and lets check-plan flag pictures left un-handled (the wrong-project /
    patient-photo risk).
    """
    ...
    for order, container in enumerate(_picture_containers(slide_root), start=1):
        ...
        embed_rid = _blip_embed_rid(container)
        rel = relationships.get(embed_rid) if embed_rid else None
        media_target = ""
        if rel and rel.get("type") == IMAGE_REL_TYPE:
            media_target = _normalize_part(rel["target"], slide_ref.part_name)
        pictures.append(
            {
                "image_id": f"s{slide_ref.index:02d}_pic{shape_id}",
                "shape_id": shape_id,
                "shape_name": shape_name,
                "embed_rid": embed_rid,
                "media_target": media_target,
                "alt": _shape_description(container),
...
def analyze_pptx(pptx_path: Path) -> dict[str, Any]:
    ...
    with zipfile.ZipFile(pptx_path) as zf:
        ...
        for slide_ref in slide_refs:
            ...
            pictures = _analyze_pictures(zf, slide_root, slide_ref)
                    ...
                    "image_edits": [
                        {
                            "image_id": "s01_pic5",
                            "action": "clear",
                            "label": "［配图：在此放置贴合新内容的图］",
                        },
                        {
                            "image_id": "s01_pic6",
                            "action": "replace",
                            "new_image": "/abs/path/to/new_image.png",
                        },
                    ],
                }
            ],

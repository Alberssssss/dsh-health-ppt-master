...
from .image_fill import _apply_image_edits_to_slide_package
from .notes import (
    _ensure_notes_master,
    _find_notes_master_target,
    _slide_rels_with_notes,
    _verify_standard_notes_package,
)
...
def apply_plan(
    ...
    *,
    ...
    has_new_notes = any(
        str(item.get("notes") or item.get("speaker_notes") or "").strip()
        for item in plan_slides
    )
    if has_new_notes:
        notes_master_target = _ensure_notes_master(
            entries,
            pres_root,
            pres_rels_root,
            content_root,
    ...
    next_media_number = _max_media_part_number(entries)
    ...
    for offset, item in enumerate(plan_slides):
        ...
        image_edits = item.get("image_edits", [])
        if not isinstance(image_edits, list):
            raise RuntimeError(f"Slide {source_slide} image_edits must be a list")
        next_media_number = _apply_image_edits_to_slide_package(
            slide_root,
            slide_rels_root,
            entries,
            content_root,

...
def _add_content_type_default(content_root: ET.Element, extension: str, content_type: str) -> None:
    """Ensure a ``<Default Extension=… ContentType=…>`` rule exists (idempotent).

    Media parts (png / jpeg / …) are typed by extension Defaults, not per-part
    Overrides. A replacement image whose extension already has a Default needs
    nothing; a new extension gets one added so the part is well-typed.
    """
...
def _max_media_part_number(entries: dict[str, bytes]) -> int:
    """Highest N across ``ppt/media/imageN.*`` so replacements mint fresh names.

    Replacement media must be a NEW part per output slide: ``clone.py`` keeps
    media parts shared across cloned slides, so overwriting a shared part would
    bleed the new image onto every slide reusing that source picture.
    """
    max_number = 0

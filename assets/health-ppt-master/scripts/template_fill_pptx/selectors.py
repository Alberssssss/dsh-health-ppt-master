...
def _image_selectors(image_edit: dict[str, Any]) -> list[str]:
    selectors = []
    if image_edit.get("image_id"):
        selectors.append(f"image_id:{image_edit['image_id']}")
    if image_edit.get("shape_id"):
        selectors.append(f"shape_id:{image_edit['shape_id']}")
    if image_edit.get("shape_name"):
        selectors.append(f"shape_name:{image_edit['shape_name']}")
    return selectors

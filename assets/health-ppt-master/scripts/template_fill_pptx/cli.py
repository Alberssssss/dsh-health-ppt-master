...
def build_parser() -> argparse.ArgumentParser:
    ...
    apply.add_argument(
        "--allow-source-images",
        "--allow-reuse",
        dest="allow_source_images",
        action="store_true",
        help=(
            "Override the image gate. By default apply REFUSES (exit 2, no file written) when a "
            "cloned source slide keeps an embedded picture that no image_edit clears or replaces "
            "— those pictures ride into the new deck verbatim (wrong-project photos / patient "
            "imagery). Pass this only after confirming every kept picture is generic "
            "(logo / decorative). (--allow-reuse is kept as a backward-compatible alias.)"
        ),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "apply":
            pptx_path = Path(args.pptx_file).expanduser().resolve()
            plan = _load_json(Path(args.plan_json).expanduser().resolve())
            if not args.allow_source_images:
                blocking = image_block_report(pptx_path, plan)
                if blocking:
                    print(format_image_refusal(blocking), file=sys.stderr)
                    return 2

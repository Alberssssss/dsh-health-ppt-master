...
def _xml_bytes(root: ET.Element) -> bytes:
    # ElementTree's namespace registry is global. Relationship writers need
    # REL_NS as the default namespace, while [Content_Types].xml must serialize
    # as `<Types xmlns="…content-types">`, not `<ns0:Types …>`. The latter is
    # namespace-equivalent XML but is rejected by LibreOffice and some strict
    # iOS/Office package readers.
    if root.tag == _qn(CT_NS, "Types"):
        ET.register_namespace("", CT_NS)
    elif root.tag == _qn(REL_NS, "Relationships"):
        ET.register_namespace("", REL_NS)
...
def _picture_containers(slide_root: ET.Element) -> list[ET.Element]:
    """All ``<p:pic>`` picture shapes on the slide, at any nesting depth.

    A ``<p:pic>`` always carries an embedded image via
    ``<p:blipFill><a:blip r:embed="rIdN"/>``; charts / tables are different
    shapes and are intentionally not returned here.
    """
    return slide_root.findall(".//p:pic", NS)


def _blip_embed_rid(picture: ET.Element) -> str:
    """The ``r:embed`` (or ``r:link``) rId of a picture's blip fill, or ""."""
    embed = _qn(NS["r"], "embed")
    link = _qn(NS["r"], "link")
    for blip in picture.findall(".//a:blip", NS):
        rid = blip.attrib.get(embed) or blip.attrib.get(link)
        if rid:
            return rid
    return ""


def _shape_description(container: ET.Element) -> str:
    """The shape's alt-text description (``cNvPr/@descr`` or ``@title``), or ""."""
    c_nv_pr = container.find(".//p:cNvPr", NS)
    if c_nv_pr is None:
        return ""
    return (c_nv_pr.attrib.get("descr") or c_nv_pr.attrib.get("title") or "").strip()

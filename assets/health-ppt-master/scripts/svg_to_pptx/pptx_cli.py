...
# health-fork: quality-sentinel gate helpers (see svg_quality_checker.py).
# The checker writes <project>/.svgqc_pass.json on a clean (0-error) run; export
# refuses (Phase B) / warns (Phase A) unless a sentinel exists whose content hash
# matches the SVGs about to be exported. The hash detects "checked an earlier
# version, then edited an SVG, then exported".
SVGQC_SENTINEL_NAME = '.svgqc_pass.json'


def svgqc_content_hash(files: list[Path]) -> str:
    """sha256 over the sorted SVG file contents (path-stem + bytes).

    Must stay byte-for-byte identical to the hash svg_quality_checker.py writes
    into the sentinel, or every export would see a false mismatch.
    """
    h = hashlib.sha256()
...
''',
    ...
    # health-fork: notes delivery gate. By default, export REFUSES (exit 1) when
    # notes/ holds per-page .md files that do not all map to slides — a mis-named
    # hand split (e.g. numeric 01.md vs SVG stem 01_封面) otherwise silently ships
    # a note-less deck. Pass this only for a genuinely note-less deck.
    parser.add_argument('--allow-missing-notes', action='store_true',
                        help='health-fork: allow export when notes/ has files that do not '
                             'map to slides (or maps only some). Default: refuse (exit 1).')
    # health-fork: quality-checker sentinel gate (Phase A = warn-only).
    parser.add_argument('--skip-quality-check', action='store_true',
                        help='health-fork: bypass the svg_quality_checker sentinel gate.')
    ...
    # health-fork: notes delivery gate (mirrors the --recorded-narration gate below).
    # Distinguishes "deck legitimately has no notes" (exit 0) from "notes exist on
    # disk but don't map to slides" (exit 1). The note discovery only matches an
    # exact SVG stem or a slideNN name, so hand-numbered NN.md files map to nothing
    # and would otherwise be silently dropped (20 slides / 0 notes / clean exit).
    if enable_notes and not args.allow_missing_notes:
        notes_dir = project_path / 'notes'
        if notes_dir.is_dir():
            md_files = [p for p in sorted(notes_dir.glob('*.md')) if p.name != 'total.md']
            if md_files and not notes:
                print(
                    "Error: notes/ contains per-page .md files but NONE map to a slide "
                    f"(matched 0/{len(ref_files)}). Hand-numbered files like 01.md are "
                    "silently dropped — note files must be named to their SVG stem "
                    "(e.g. 01_封面.md) or slideNN.",
                    file=sys.stderr,
                )
                for p in md_files[:20]:
                    print(f"  Unmapped note file: {p.name}", file=sys.stderr)
                if len(md_files) > 20:
                    print(f"  ... and {len(md_files) - 20} more", file=sys.stderr)
                print(
                    "  Fix: python3 scripts/total_md_split.py <project_path>  "
                    "(splits notes/total.md and names each note to its SVG stem). "
                    "Or pass --allow-missing-notes for a genuinely note-less deck.",
                    file=sys.stderr,
                )
                sys.exit(1)
            elif notes:
                missing = [p.stem for p in ref_files if p.stem not in notes]
                if missing:
                    print(
                        "Error: speaker notes are incomplete: "
                        f"matched {len(notes)}/{len(ref_files)} slide(s). "
                        "Every slide must have a matching note file (or pass "
                        "--allow-missing-notes).",
                        file=sys.stderr,
                    )
                    for stem in missing[:20]:
                        print(f"  Missing note for: {stem}", file=sys.stderr)
                    if len(missing) > 20:
    ...
    # health-fork: quality-checker sentinel gate. Now a HARD gate (was Phase-A
    # warn-only): the SOP prose this was waiting on has shipped, so a native
    # export refuses unless svg_quality_checker.py passed cleanly for the exact
    # svg_output/ being exported. Freshness = the sentinel's content_hash matches
    # those SVGs, so editing an SVG after checking invalidates it. finalize_svg.py
    # writes to svg_final/ (copytree) and leaves svg_output/ untouched, so the
    # sentinel stays fresh across the normal QC -> finalize -> export flow.
    # Escape hatch: --skip-quality-check for deliberate re-exports.
    if gen_native and not args.skip_quality_check:
        sentinel = project_path / SVGQC_SENTINEL_NAME
        fresh = False
        if sentinel.is_file():
            try:
                recorded = json.loads(sentinel.read_text(encoding='utf-8')).get('content_hash')
            except (json.JSONDecodeError, OSError):
                recorded = None
            fresh = bool(recorded) and recorded == svgqc_content_hash(native_files)
        if not fresh:
            reason = "no sentinel" if not sentinel.is_file() else "sentinel is stale (svg_output changed since the last check)"
            print(
                f"  [error] svg_quality_checker has not passed for the current svg_output/ ({reason}). "
                "Run: python3 scripts/svg_quality_checker.py <project_path>  before export "
                "(or pass --skip-quality-check to override).",
                file=sys.stderr,
            )
            sys.exit(1)

...
def _relationship_entries(rels_path: Path) -> list[dict[str, str]]:
    """Return relationship attributes from *rels_path*."""
    if not rels_path.exists():
        return []
    root = ET.parse(rels_path).getroot()
    return [dict(rel.attrib) for rel in root]


def _relationship_types(rels_path: Path) -> set[str]:
    """Return relationship types declared in *rels_path*."""
    return {
        rel.get('Type', '')
        for rel in _relationship_entries(rels_path)
        if rel.get('Type')
    }


def _ensure_notes_master_id_list(extract_dir: Path) -> None:
    """Make presentation.xml reference its singleton notesMaster relationship.

    ``python-pptx`` creates the notesMaster relationship and parts but currently
    omits ``p:notesMasterIdLst``. Desktop readers repair that omission; iOS
    previewers can reject the notes-bearing package. Normalize it before export.
    """
    presentation_rels = extract_dir / 'ppt' / '_rels' / 'presentation.xml.rels'
    master_rels = [
        rel
        for rel in _relationship_entries(presentation_rels)
        if rel.get('Type') == _NOTES_MASTER_REL_TYPE
    ]
    if len(master_rels) != 1 or not master_rels[0].get('Id'):
        raise RuntimeError(
            'Speaker notes require exactly one presentation notesMaster relationship'
        )
    master_rid = master_rels[0]['Id']

    presentation_path = extract_dir / 'ppt' / 'presentation.xml'
    tree = ET.parse(presentation_path)
    root = tree.getroot()
    list_tag = _qn(_PML_NS, 'notesMasterIdLst')
    id_tag = _qn(_PML_NS, 'notesMasterId')
    notes_list = root.find(list_tag)
    if notes_list is None:
        notes_list = ET.Element(list_tag)
        children = list(root)
        insert_at = next(
            (
                index
                ...
                if child.tag in {
                    _qn(_PML_NS, 'handoutMasterIdLst'),
                    _qn(_PML_NS, 'sldIdLst'),
                }
            ),
            len(children),
        )
        ...
        for child in list(notes_list):
            notes_list.remove(child)
    ET.SubElement(
        notes_list,
        id_tag,
        {_qn(_DOC_REL_NS, 'id'): master_rid},
    )
    tree.write(
        presentation_path,
        encoding='UTF-8',
...
def _verify_notes_package(
    extract_dir: Path,
    notes_parts_by_slide: dict[int, str],
) -> list[str]:
    """Return PowerPoint-standard notes-package compatibility problems."""
    if not notes_parts_by_slide:
        return []
    ...
    master_rid = master_rels[0].get('Id', '') if len(master_rels) == 1 else ''
    master_target = master_rels[0].get('Target', '') if len(master_rels) == 1 else ''
    if len(master_rels) != 1 or not master_rid or not master_target:
        problems.append(
            'presentation.xml.rels must have exactly one notesMaster relationship'
        )
    if master_target.startswith('/'):
        master_part = master_target.lstrip('/')
    else:
        master_part = posixpath.normpath(posixpath.join('ppt', master_target))
    notes_master = extract_dir / master_part if master_part else Path()
    notes_master_rels = (
        extract_dir / posixpath.dirname(master_part) / '_rels'
    ...
    if presentation_path.exists():
        ...
        notes_ids = presentation_root.findall(
            f'./{_qn(_PML_NS, "notesMasterIdLst")}/{_qn(_PML_NS, "notesMasterId")}'
        )
        notes_id_rids = [
            node.attrib.get(_qn(_DOC_REL_NS, 'id'), '') for node in notes_ids
        ]
        if len(notes_ids) != 1 or notes_id_rids != [master_rid]:
            problems.append(
                'presentation.xml notesMasterIdLst must contain the notesMaster relationship Id'
            )
    else:
        problems.append('ppt/presentation.xml is missing')

    if not master_part or not notes_master.exists():
        problems.append(f'{master_part or "notesMaster part"} is missing')
    else:
        master_root = ET.parse(notes_master).getroot()
        if master_root.find(_qn(_PML_NS, 'notesStyle')) is None:
            problems.append(f'{master_part} has no p:notesStyle')

    theme_rels = [
        rel
        for rel in _relationship_entries(notes_master_rels)
        if rel.get('Type') == _THEME_REL_TYPE
    ]
    if len(theme_rels) != 1 or not theme_rels[0].get('Target'):
        problems.append(
            f'{posixpath.basename(master_part) or "notesMaster"}.rels must have exactly one theme relationship'
        )
    elif master_part:
        theme_target = theme_rels[0]['Target']
        theme_part = posixpath.normpath(
            posixpath.join(posixpath.dirname(master_part), theme_target)
        )
        if not (extract_dir / theme_part).exists():
            problems.append(f'{theme_part} is missing')

    content_types_path = extract_dir / '[Content_Types].xml'
    content_root = ET.parse(content_types_path).getroot()
    content_overrides = {
        node.attrib.get('PartName', '').lstrip('/'): node.attrib.get('ContentType', '')
        for node in content_root.findall(_qn(_CONTENT_TYPES_NS, 'Override'))
    }
    if master_part and content_overrides.get(master_part) != _NOTES_MASTER_CONTENT_TYPE:
        problems.append(f'[Content_Types].xml has no valid {master_part} override')
    invalid_notes_rels_overrides = sorted(
        part
        for part in content_overrides
        if part.endswith('.rels')
        and part.startswith(('ppt/notesSlides/', 'ppt/notesMasters/'))
    )
    if invalid_notes_rels_overrides:
        problems.append(
            '[Content_Types].xml must not override notes relationship parts: '
            + ', '.join(invalid_notes_rels_overrides)
        )

    for slide_num, notes_part in sorted(notes_parts_by_slide.items()):
        notes_name = Path(notes_part).name
        notes_slide = extract_dir / notes_part
        notes_rels = (
            extract_dir / 'ppt' / 'notesSlides' / '_rels'
            / f'{notes_name}.rels'
        )
        slide_rels = (
        ...
        if not notes_slide.exists():
            problems.append(f'{notes_part} is missing')
        else:
            notes_root = ET.parse(notes_slide).getroot()
            placeholder_types = {
                node.attrib.get('type', '')
                for node in notes_root.iter(_qn(_PML_NS, 'ph'))
            }
            missing_placeholders = {'sldImg', 'body', 'sldNum'} - placeholder_types
            if missing_placeholders:
                problems.append(
                    f'{notes_name} is missing standard placeholders: '
                    + ', '.join(sorted(missing_placeholders))
                )
            if notes_root.find(_qn(_PML_NS, 'clrMapOvr')) is None:
                problems.append(f'{notes_name} has no p:clrMapOvr')

        note_relationships = _relationship_entries(notes_rels)
        note_master_rels = [
            rel for rel in note_relationships
            if rel.get('Type') == _NOTES_MASTER_REL_TYPE
        ]
        note_slide_rels = [
            rel for rel in note_relationships
        ...
        if len(note_master_rels) != 1:
            problems.append(f'{notes_name}.rels must have one notesMaster relationship')
        elif master_part:
            resolved_master = posixpath.normpath(
                posixpath.join('ppt/notesSlides', note_master_rels[0].get('Target', ''))
            )
            if resolved_master != master_part:
                problems.append(
                    f'{notes_name}.rels targets {resolved_master}, expected {master_part}'
                )
        if len(note_slide_rels) != 1:
            problems.append(f'{notes_name}.rels must have one slide relationship')
        else:
            resolved_slide = posixpath.normpath(
                posixpath.join('ppt/notesSlides', note_slide_rels[0].get('Target', ''))
            )
            expected_slide = f'ppt/slides/slide{slide_num}.xml'
            if resolved_slide != expected_slide:
                problems.append(
                    f'{notes_name}.rels targets {resolved_slide}, expected {expected_slide}'
                )

        slide_note_rels = [
            rel
            for rel in _relationship_entries(slide_rels)
            if rel.get('Type') == _NOTES_SLIDE_REL_TYPE
        ]
        if len(slide_note_rels) != 1:
            problems.append(
                f'slide{slide_num}.xml.rels must have one notesSlide relationship'
            )
        else:
            resolved_notes = posixpath.normpath(
                posixpath.join('ppt/slides', slide_note_rels[0].get('Target', ''))
            )
            if resolved_notes != notes_part:
                problems.append(
                    f'slide{slide_num}.xml.rels targets {resolved_notes}, expected {notes_part}'
                )
        if content_overrides.get(notes_part) != _NOTES_SLIDE_CONTENT_TYPE:
            problems.append(f'[Content_Types].xml has no valid {notes_name} override')
...
def create_pptx_with_native_svg(
    ...
    try:
        ...
        prepared_notes_by_slide: dict[int, str] = {}
        prepared_notes_parts_by_slide: dict[int, str] = {}
        for slide_num, svg_path in enumerate(svg_files, 1):
            slide = prs.slides.add_slide(blank_layout)
            if not enable_notes:
                continue

            notes_content = notes.get(svg_path.stem, '') if notes else ''
            notes_text = markdown_to_plain_text(notes_content) if notes_content else ''
            if not notes_text:
                continue

            # Let python-pptx create the complete notes package before we
            # rewrite slide DrawingML below. Accessing ``notes_slide`` creates
            # the singleton notesMaster, its theme, Content Types overrides,
            # and both notesMaster/slide relationships. The previous exporter
            # hand-wrote only notesSlides, which desktop renderers tolerated
            # but iOS Quick Look could treat as an unreadable/blank deck.
            notes_slide = slide.notes_slide
            notes_text_frame = notes_slide.notes_text_frame
            if notes_text_frame is None:
                raise RuntimeError(
                    f'Unable to create a notes text frame for slide {slide_num}'
                )
            notes_text_frame.text = notes_text
            prepared_notes_by_slide[slide_num] = notes_text
            prepared_notes_parts_by_slide[slide_num] = str(
                notes_slide.part.partname
            ).lstrip('/')
        ...
        if prepared_notes_parts_by_slide:
            _ensure_notes_master_id_list(extract_dir)
        ...
        notes_slides_created: dict[int, str] = {}
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slideLayout" Target="../slideLayouts/slideLayout1.xml"/>{extra_rels}
                ...
                if slide_num in prepared_notes_by_slide:
                    # The notesSlide and its complete notesMaster relationship
                    # already came from the python-pptx base package. Reattach
                    # only the slide -> notesSlide edge because this exporter
                    # replaces each slide's .rels while writing slide content.
                    notes_part = prepared_notes_parts_by_slide[slide_num]
                    _append_relationship(
                        rels_path,
                        'http://schemas.openxmlformats.org/officeDocument/2006/relationships/notesSlide',
                        f'../notesSlides/{Path(notes_part).name}',
                    )
                    notes_slides_created[slide_num] = notes_part
        ...
        if enable_notes and notes_slides_created:
            for notes_part in sorted(set(notes_slides_created.values())):
                override = (
                    f'  <Override PartName="/{notes_part}" '
                    f'ContentType="application/vnd.openxmlformats-officedocument.presentationml.notesSlide+xml"/>'
                )
                if f'PartName="/{notes_part}"' not in content_types:
                    content_types = content_types.replace('</Types>', override + '\n</Types>')
            with open(content_types_path, 'w', encoding='utf-8') as f:
                f.write(content_types)

        notes_problems = _verify_notes_package(extract_dir, notes_slides_created)
        if notes_problems:
            details = '\n'.join(f'  - {p}' for p in notes_problems)
            raise RuntimeError(
                'PPTX package contains an incomplete speaker-notes structure; '
                'iOS preview may render the deck as blank:\n' + details
            )

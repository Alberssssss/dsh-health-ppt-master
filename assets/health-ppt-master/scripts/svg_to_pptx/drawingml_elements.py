...
def convert_image(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    if w <= 0 or h <= 0:
        # A genuinely unsized <image> (no width/height attrs) resolves to 0 via
        # _f()'s default and is a legitimate skip. But an <image> whose width AND
        # height were explicitly authored yet compute to a non-positive box is a
        # broken embed — fail loudly (convert_element wraps this into a
        # SvgNativeConversionError) instead of silently dropping the figure.
        if elem.get('width') is not None and elem.get('height') is not None:
            raise ValueError(
                f'<image> has an explicit but non-positive display box (w={w}, h={h}); '
                f'href={href[:80]!r}'
            )
        return None
    ...
    if href.startswith('data:'):
        ...
        if not match:
            # A data: URI that is not decodable base64 image data is corrupt —
            # raise rather than silently drop it (mirrors the missing-file raise).
            raise ValueError(
                f'<image> has a malformed data: URI (not base64 image data): {href[:60]!r}...'
            )

...
export const ROUTER_SOURCE = 'health-ppt-master'

/** Stable model-visible reminder emitted for a matching user request. */
export const ROUTER_HINT = [
...
const DECK_TERMS = [
  'ppt', 'pptx', 'powerpoint', '幻灯', '演示文稿', '课件',
...
const READ_ONLY_TERMS = [
  ...
  '有几页', '多少页', '讲了什么', '讲了啥', '内容是什么',
  'read ', 'summarize', 'extract', 'export', 'convert', 'how many slides',
] as const

const PRESENTATION_DELIVERABLES = [
  '汇报材料', '路演', '做演示', '工作汇报', '科室汇报', '年度总结', '季度汇报',
  '述职', '进修汇报', '病例汇报', '病例讨论', '疑难病例', '病历汇报',
  '教学查房', '查房课件', '开题报告', '中期汇报', '结题汇报', '文献汇报',
  '读书报告会', '学术汇报', '大会发言', '学术幻灯', '答辩', '医学科普',
  '职称申报', '职称晋升材料', '科技奖申报', '等级评审', '学科评审',
  '药械申报', '注册申报', 'case report ppt', 'clinical case ppt',
  'journal club deck', 'grant proposal slides',
] as const

const TEMPLATE_ACTIONS = [
  '套用模板', '套用这个', '按这个模板', '按模板', '用这个模板', '模板填充',
  '填进模板', '填到模板', '参照模板', '参照这个ppt', '参照这份ppt',
  '仿照这个ppt', '仿照这份ppt', '参考这个模板', '参考这份模板',
  '参考这个ppt', '参考这份ppt', '参考上传的ppt', '照着这个ppt', '照这个ppt',
  '按这个ppt', '按这份ppt', '医院模板', '科室模板', '本院模板', '院内模板',
] as const

const DOMAIN_MARKERS = [
  '科室', '医院', '临床', '病区', '门诊', '病房', '医师', '主任医师', '医学',
  '科研', '医生', '我科', '本科室', '院内', '学科',
] as const

const DOMAIN_DELIVERABLES = [
  '工作汇报', '科室汇报', '课件', '幻灯', '演示', '述职', '答辩', '病例汇报',
 ...
 */
export function matchesHealthPptMaster(value: string): boolean {
 ...
 */
export function matchesHealthPptMasterMessages(messages: readonly UserMessage[]): boolean {
  return messages.some(message => message.source.kind === 'user'
    && matchesHealthPptMaster(textOf(message)))
}
 ...
 */
export function registerHealthPptMasterRouter(ctx: Context): void {
  ctx.on('agent/pre-step', async ({ messages, signal }, next): Promise<PreStepDecision> => {
    ...
    return {
      ...
      messages: [
        ...decision.messages,
        createUserMessage({
          content: [{ type: 'text', text: ROUTER_HINT }],
